...
class VocDataset(Dataset):
    ...
    def __parse_annotation(self, annotation):
        ...
        # 将该图片中不同目标的坐标+目标的类分给bboxes，其中bboxes是一个列表，也就是同一图片中可以有多个目标
        bboxes = np.array([list(map(float, box.split(','))) for box in anno[1:]])

        # 对内存中的图片进行4次数据增强
        # 最后返回增强后的图片以及图片中各目标的box
    ...
    def __creat_label(self, bboxes):
        ...
        for bbox in bboxes:
            ...
            # 将bbox坐标形式为xywh的，分别除以stride，即8/16/32，得到不同尺度的GTbox，即真实标签框
        ...
        # 我感觉是将.csv的一行中的各目标标签、box给分成3种不同尺度，仅此而已
